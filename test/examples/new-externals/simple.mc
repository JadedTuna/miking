...
let ext_pow = externalbind "Float.pow" (lam a : Float -> Float -> Float. ()) []
let ext_rnd_init = externalbind "Random.self_init" (lam a : () -> (). ()) []
let ext_rnd_int = externalbind "Random.int" (lam a : Int -> Int. ()) []

-- input arguments are ignored, 9 and 2 are used instead
let ext_pow_9_2 = externalbind "let foo a b = Float.pow 9.0 2.0 in foo" (lam a : Float -> Float -> Float. ()) []
